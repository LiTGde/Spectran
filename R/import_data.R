...
import_dataServer <-
  function(id, Spectrum = NULL) {
    shiny::moduleServer(id, function(input, output, session) {
      ...
      file_validation_active <- shiny::reactiveVal(FALSE)
      ...
      shiny::observe({
        ...
        shiny::bindEvent(input$jgtm, ignoreInit = TRUE)
      ...
      shiny::observe({
        ...
        shiny::bindEvent(input$in_file$datapath, ignoreInit = TRUE)

      #Only an explicit file selection may activate the legacy file validator.
      #Built-in examples and constructed spectra share this module's global
      #Spectrum state, but must never inherit file-validation notifications.
      shiny::observeEvent(
        Spectrum$Origin,
        {
          if (!identical(Spectrum$Origin, "File")) {
            file_validation_active(FALSE)
          }
        },
        ignoreInit = FALSE
      )
      ...
      shiny::observe({
        shiny::updateTextInput(
          session,
          "name_id",
          value = split_filename(input$in_file$name)
        )
      }) %>%
        shiny::bindEvent(input$in_file$datapath, ignoreInit = TRUE)
      ...
      dat0 <- shiny::reactive({
        ...
        temp <-
          try(
            read_spectral_csv(importfile(), csv_settings()),
            silent = TRUE
          )
        temp
      })
        ...
        dat({
          ...
          if (dplyr::between(csv_settings()$x_y2, 1, ncol(temp))) {
            if (
              ...
              temp[
                (temp[[csv_settings()$x_y2]] < 0 & !is.na(temp[[2]])),
                csv_settings()$x_y2
              ] <- 0
            }
          }
