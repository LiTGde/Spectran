...
Spec_wtd <- function(spectrum, sensitivity) {
  spectral_weighted_values(spectrum, sensitivity)
}

#creates the integral of the spectrum

Spec_int <- function(spectrum, sensitivity) {
  sum(spectral_weighted_values(spectrum, sensitivity))
}
...
PD <- function(wavelength, irradiance) {
  ...
  const_h <- 6.626 * 10^-34
  const_c <- 2.998 * 10^8
  (irradiance * wavelength * 10^-9) / (const_h * const_c) / 100^2
}
...
checkbox_update <- function(name, test) {
  ...
  if (test) {
    ...
    shinyjs::html(
      "r_label",
      paste0("<p style='color:red'>", lang$server(48), "</p>")
    )
  } else {
    ...
    shinyjs::html(
      "r_label",
      paste0("<p style='color:black'>", lang$ui(99), "</p>")
    )
  }
}
...
cS_object <- function(Spectrum) {
  Spectrum[[2]] %>%
    colorSpec::colorSpec(wavelength = 380:780, quantity = "energy")
}
...
table_alpha <- function(Analysis) {
  tibble::tibble(
    ...
    Zeichen = c(
      "E<sub>v,&alpha;,D65</sub>",
      "E<sub>e,&alpha;</sub>",
      "a<sub>&alpha;,v</sub>",
      "&gamma;<sub>&alpha;,v,D65</sub>"
    ),
    Formelzeichen = c("E_v,a,D65", "E_e,a", "a_a", "y_a,v,D65"),
    purrr::map_df(Specs$Plot$Names[1:5], \(x) {
      Analysis[[ns_table(x)]]$internal$Wert
    }),
    Einheit = c("lux", "mW/m\u00b2", "", "")
  )
}
