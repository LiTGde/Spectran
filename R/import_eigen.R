...
import_eigenServer <-
  function(
    ...
    shiny::moduleServer(id, function(input, output, session) {
      ...
      eigen_Spectrum <- import_eigen_plotServer(
        "plot",
        Spectrum = Spectrum,
        sensitivitaeten = shiny::reactive(input$sensitivitaeten),
        ...
        Spectrum$Other <-
          if (input$is_illu_eigen) {
            Spectrum$Spectrum_raw <-
              eigen_Spectrum() %>%
              dplyr::mutate(
                Bestrahlungsstaerke = Bestrahlungsstaerke /
                  Ev *
                  input$illu_eigen
              )
          } else if (
            ...
            Spectrum$Spectrum_raw <-
              eigen_Spectrum() %>%
              dplyr::mutate(
                Bestrahlungsstaerke = Bestrahlungsstaerke * Spectrum$Emax_orig
              )
          } else {
            Spectrum$Spectrum_raw <-
              eigen_Spectrum() %>%
      ...
      shiny::observe({
        if (
          all(
            (Spectrum$Destination) == lang$ui(94),
            !is.null((Spectrum$Spectrum))
          )
        ) {
