...
import_examples_boxUI <-
  function(id, title = "", left = NULL, mid = NULL, right = NULL) {
    ns <- shiny::NS(id)
  ...
  function(
    ...
    shiny::moduleServer(id, function(input, output, session) {
      ...
      save_from_one <- function() {
        ...
        dataframe <- examplespectra_descriptor %>%
          tidyr::unnest(
          ...
          \(name, filename, Button_Name) {
            shiny::observe({
              ...
              if (
                ...
                Spectrum$Name <- {
                  paste0(
                    examplespectra_descriptor$Beschreibung,
                    ": ",
                    Button_Name
                  )
                }
      ...
      save_from_many <- function() {
        ...
        shiny::observe({
          #CIE Download:
          if (
            all(down_import() == "Download", examplespectra_descriptor$embargo)
          ) {
            ...
            Spectrum$Destination <- lang$ui(69)
            Spectrum$Origin <- if (examplespectra_descriptor$embargo) "CIE" else
              "Example"
            signal_spectran_import_attempt(Spectrum)
          }
        }) %>%
      ...
      if (id == "norm") {
        ...
        daylight_spectrum <- shiny::reactive({
          Bspdat <- tibble::tibble(
            ...
            Bestrahlungsstaerke = colorSpec::daylightSpectra(
              daylight_CCT(),
              wavelength = Wellenlaenge
            )
          ) %>%
            dplyr::mutate(
              Bestrahlungsstaerke = unclass(Bestrahlungsstaerke) /
                Calc_lux(
                  Bestrahlungsstaerke,
                  Specs$AS_wide,
                  Specs$Efficacy
        ...
        shiny::observe({
          ...
          } else if (down_import() == "Import") { #Import for Daylight:
            shiny::observe({
      ...
      } else if (length(examplespectra_descriptor[["Identifier"]][[1]]) > 1) { #Handling when there are many Spectra (Selector)
        save_from_many()
      } else { #Handling when there is one button per Spectrum
        save_from_one()
      }
    })
  }
